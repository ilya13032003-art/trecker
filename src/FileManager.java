...
public class FileManager {
    ...
    public void stringToTask(String[] arr, HashMap<Integer, Task> baseTask,
        ...
        if (TaskType.TASK.equals(type)) {
            baseTask.put(id, new Task(arr[2], arr[3], id, status, type));
        } else if (TaskType.SUB_TASK.equals(type)) {
            int epicId = Integer.parseInt(arr[5]);
            for (Epic epic : baseEpic.values()) {
                if (epicId == epic.id) {
                    epic.getSubTaskArray().put(id, new Task(arr[2], arr[3], id, status, type));
                }
            }
